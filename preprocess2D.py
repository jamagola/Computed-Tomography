...
out_file=open(out_file_path+'preprocess2D.out','a+')
...
target_param=percent_foreigns #change target here
...
for i in folder:
  ...
  for j in np.arange(20,799,step):
    ...
    target.append(target_param[i.split('_')[1]]) #Change target here, e.g. peak_stress 
    lotmap.append(i.split('_')[1]) 
    count=count+1 
  
data=image.reshape(-1,300,300,1)
...
np.save(out_file_path+'target2D.npy', target)
np.save(out_file_path+'CTdata2D.npy',data)
with open(out_file_path+'lotmap2D.pkl','wb') as f:
